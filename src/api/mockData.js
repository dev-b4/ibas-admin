...
export const fetchIbasData = async () => {
  ...
  try {
    ...
    // --- NOVA LÓGICA DE PESOS (CUSTÓDIA = 50% DA BASE) ---
    const isListed = (s) => ['Listado', 'Listed'].includes(s);
    const isCustody = (s) => ['Em Custódia', 'Custódia', 'Custodiado', 'Custodied'].includes(s);
    
    // Only count projects that are either listed or custody towards the valid index pool
    const validProjects = dbProjects.filter(p => isListed(p.status) || isCustody(p.status));
    
    const totalListed = validProjects.filter(p => isListed(p.status)).length;
    const totalCustody = validProjects.filter(p => isCustody(p.status)).length;
    ...
    if (totalProjects > 0) {
      if (totalListed === 0) {
        custodyWeight = 1 / totalCustody;
      } else if (totalCustody === 0) {
        listedWeight = 1 / totalListed;
      } else {
        const baseWeight = 1 / totalProjects;
        custodyWeight = baseWeight / 2;
        const custodyTotal = custodyWeight * totalCustody;
        listedWeight = (1 - custodyTotal) / totalListed;
      }
    }

    const getDynamicWeight = (status) => {
      if (isListed(status)) return listedWeight;
      if (isCustody(status)) return custodyWeight;
